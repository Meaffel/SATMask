...
for k in [7, 4, 2, 1]:
    ...
    if k == 7:
        ...
        model.load_state_dict(torch.load("mnist_cnn_k4.pth", map_location=device))
    elif k == 2:
        model = SimpleCNN_28x28_k2().to(device)
        model.load_state_dict(torch.load("mnist_cnn_k2.pth", map_location=device))
    elif k == 1:
        model = SimpleCNN_28x28_k1().to(device)
        model.load_state_dict(torch.load("mnist_cnn_k1.pth", map_location=device))
